function [x,pointlist] = PreConjGrad(func,input,x0,V,t,itmax,tol)
...
while ( iter < itmax )
    ...
    if ( cycle == 0 )
       d = -grad*V;
        ...
        alpha = dot((grad-gold)*V,grad)/dot(gold,gold*V);
        d = -grad*V + alpha*dold;
        s = dot(d,grad);
        if ( s >= -tol^2 ) 
            d = -grad*V;
    ...
    toldir = tol*norm(d);
    [x,t0] = DirMin(func,x,input,d,s,val,t0/norm(d),toldir);
